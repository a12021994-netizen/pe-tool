...
function buildChartSVG(rows, dailyHistory){
  ...
  const W = 640, H = 250, padL = 50, padR = 50, padT = 16, padB = 46;
  ...
  points.forEach(p=>{
    ...
    if (x - lastLabelX >= minLabelGap){
      svg += `<text x="${x}" y="${H-10}" text-anchor="end" font-size="7" fill="var(--text3)" transform="rotate(-35 ${x} ${H-10})">${p.reportDate.slice(2)}</text>`;
      lastLabelX = x;
    }
  });
  svg += `</svg>`;
