...
function renderAddSnapForm(){
  ...
  f.innerHTML = `
    <div class="row">
      ...
      <div><span class="label">當時股價</span><input type="number" id="snapPrice" style="width:100px"></div>
      <div><span class="label">目標價 (選填)</span><input type="number" id="snapTarget" style="width:100px"></div>
    </div>
    ${pf && pf.text ? `<div class="hint" style="margin-top:8px">下方是從Drive PDF擷取出的原始文字，對照後把EPS等數字填進下面的表格：</div>
    <textarea readonly style="min-height:140px;font-family:monospace;font-size:11px">${pf.text.slice(0, 6000).replace(/</g,'&lt;')}</textarea>` : ''}
...
function renderDriveFileList(){
    ...
    return `
      <div style="border-top:0.5px solid var(--border);padding:8px 0">
        <div class="row" style="justify-content:space-between">
          ...
          <div class="row">
            <button class="small" id="drv-read-${file.id}">讀取內容</button>
            <button class="small primary" id="drv-use-${file.id}" ${parsed ? '' : 'disabled'}>建立報告快照</button>
          </div>
        </div>
  ...
  driveFiles.forEach(file=>{
    const readBtn = document.getElementById('drv-read-'+file.id);
    if (readBtn) readBtn.onclick = async ()=>{
      readBtn.disabled = true;
      readBtn.textContent = '讀取中...';
      try{
        const text = await downloadAndExtractText(file.id);
        document.getElementById('drv-text-'+file.id).innerHTML =
          `<textarea readonly style="min-height:160px;font-family:monospace;font-size:11px;width:100%;margin-top:6px">${text.slice(0,8000).replace(/</g,'&lt;')}</textarea>`;
      }catch(e){
        document.getElementById('drv-text-'+file.id).innerHTML = `<div class="hint" style="color:var(--warn)">讀取失敗：${e.message}</div>`;
      }
      readBtn.disabled = false;
      readBtn.textContent = '讀取內容';
    };
    ...
    if (useBtn) useBtn.onclick = async ()=>{
      ...
      if (!text){
        useBtn.disabled = true; useBtn.textContent = '讀取中...';
        try{ text = await downloadAndExtractText(file.id); }catch(e){ alert('讀取PDF失敗：'+e.message); useBtn.disabled=false; useBtn.textContent='建立報告快照'; return; }
        useBtn.disabled = false; useBtn.textContent = '建立報告快照';
      }
      prefillSnap = { guessedDate: parsed.guessedDate, broker: parsed.broker, text };
