...
function reconstructLines(items){
  // 用pdf.js文字項目的座標位置重建「表格列」，同一Y座標(容忍誤差)的文字視為同一行，
  // 行內再按X座標由左到右排序 —— 這樣輸出結果才會跟pdftotext -layout類似，
  // 保留原本表格的欄位對齊，而不是把整頁文字黏成一坨。
  const rows = [];
  const pts = items
  ...
  let current = null;
  pts.forEach(p=>{
    if (current && Math.abs(p.y - current.y) <= Y_TOL){
...
function autoParseReportText(text){
  ...
  const epsLineRe = /EPS/i;

  function tokensInLine(line, re){
    re.lastIndex = 0;
    return [...line.matchAll(re)];
  }

  // 逐行找「季度標籤列」(例如 "1Q26 2Q26 3Q26F 4Q26F 1Q27F 2Q27F 3Q27F 4Q27F")，
  // 找到後往下最多15行內找含「EPS」且數字數量吻合的那一列，兩者對應存成季度EPS。
  for (let i = 0; i < lines.length; i++){
    const qToks = tokensInLine(lines[i], qTokenRe);
    if (qToks.length < 4) continue;
    let found = false;
    for (let j = i; j < Math.min(i + 15, lines.length); j++){
      if (!epsLineRe.test(lines[j])) continue;
      const nums = [...lines[j].matchAll(numRe)];
      if (nums.length >= qToks.length){
        qToks.forEach((tok, k) => {
          let year, quarter;
          if (tok[1] !== undefined) { quarter = parseInt(tok[1]); year = 2000 + parseInt(tok[2]); }
          else { year = 2000 + parseInt(tok[3]); quarter = parseInt(tok[4]); }
          const isForecast = /F$/.test(tok[0]);
          result.quarters.push({ year, q: quarter, eps: parseFloat(nums[k][0]), actual: !isForecast });
        });
        found = true;
        break;
      }
    }
    if (found) break; // 採用第一個成功配對的季度表，避免同份報告裡多個表格互相干擾
  }

  // 年度EPS：同樣邏輯，找 "20xxF" 年份標籤列。跳過同一行也含季度標籤的情況
  // （那通常是單季預測表最後幾欄順帶列出的年度加總，不是獨立的年度表，
  // 直接沿用會跟季度EPS抓到同一列數字，造成誤判）。
  for (let i = 0; i < lines.length; i++){
    if (tokensInLine(lines[i], qTokenRe).length >= 1) continue;
    const yToks = tokensInLine(lines[i], yTokenRe);
    if (yToks.length < 2 || yToks.length > 6) continue;
    let found = false;
    for (let j = i; j < Math.min(i + 15, lines.length); j++){
      if (tokensInLine(lines[j], qTokenRe).length >= 1) continue;
      if (!epsLineRe.test(lines[j])) continue;
      const nums = [...lines[j].matchAll(numRe)];
      if (nums.length >= yToks.length){
        yToks.forEach((tok, k) => {
          result.annualEPS.push({ year: parseInt(tok[0]), eps: parseFloat(nums[k][0]) });
        });
        found = true;
        break;
      }
  ...
  // 股價/目標價：中文關鍵字常被拆成一字一空格（例如「前 日 收 盤 價」），
  // 用 spacedRe() 讓關鍵字每個字之間容許任意空白，才比對得到。
  function spacedRe(str){
    return str.split('').map(ch => ch.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('\\s*');
  }
  const norm = text.replace(/\n/g, ' ');
  const priceNumPattern = '(\\d{1,3}(?:,\\d{3})+(?:\\.\\d{1,2})?|\\d+(?:\\.\\d{1,2})?)';
  const priceKeywordList = ['前日收盤價','前一日收盤','最新收盤','收盤價','現價','股價'];
  const targetKeywordList = ['目標價','合理股價','目標區間'];

  for (const kw of priceKeywordList){
    const m = norm.match(new RegExp(spacedRe(kw) + '[：:\\s]*(?:NT\\$)?\\s*' + priceNumPattern));
    if (m){ result.price = parseFloat(m[1].replace(/,/g, '')); break; }
  }
  for (const kw of targetKeywordList){
    const m = norm.match(new RegExp(spacedRe(kw) + '[：:\\s]*(?:NT\\$)?\\s*' + priceNumPattern));
    if (m){ result.target = parseFloat(m[1].replace(/,/g, '')); break; }
  }
