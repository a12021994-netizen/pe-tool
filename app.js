...
function autoParseReportText(text){
  ...
  const qTokenRe = /\b(\d{1,2})Q(\d{2})F?\b|\b(\d{2})Q([1-4])F?\b/g;
  const numRe = /-?\d+\.\d{1,2}/g;

  function findBestRun(re, text){
    let m, runs = [];
    let lastEnd = -1, current = null;
    while ((m = re.exec(text))) {
      if (current && m.index - lastEnd < 4) {
  ...
  // 找季度標籤最長的一串（例如 "1Q25 2Q25F 3Q25F 4Q25F" 或 "24Q1F 24Q2F..."）
  const qRun = findBestRun(new RegExp(qTokenRe), norm);
  if (qRun && qRun.tokens.length >= 4) {
    // 在標籤區塊後方 300 字內找數量相同的數字串當作EPS
    const after = norm.slice(qRun.end, qRun.end + 300);
    ...
    if (nums.length >= qRun.tokens.length) {
      qRun.tokens.forEach((tok, i) => {
        ...
        const isForecast = /F\b/.test(tok[0]);
        result.quarters.push({ year, q: quarter, eps: parseFloat(nums[i][0]), actual: !isForecast });
      });
    }
  }

  // 年度EPS：找 "20xxF" 年份標籤串 + 附近數字
  const yTokenRe = /\b20\d{2}F?\b/g;
  const yRun = findBestRun(yTokenRe, norm);
  if (yRun && yRun.tokens.length >= 2 && yRun.tokens.length <= 5) {
    const after = norm.slice(yRun.end, yRun.end + 200);
    const nums = [...after.matchAll(numRe)];
  ...
  const priceMatch = norm.match(/(?:股價|收盤價|現價)[：:\s]*(?:NT\$)?\s*(\d{1,5}(?:\.\d{1,2})?)/);
  if (priceMatch) result.price = parseFloat(priceMatch[1]);
  const targetMatch = norm.match(/目標價[：:\s]*(?:NT\$)?\s*(\d{1,5}(?:\.\d{1,2})?)/);
...
function renderDriveFileList(){
  ...
  if (!driveFiles.length){
    box.innerHTML = '<div class="empty">尚未掃描，或資料夾內沒有PDF</div>';
    return;
  }
  box.innerHTML = driveFiles.map(file=>{
    const parsed = parseDriveFilename(file.name);
    const known = parsed && stocks.find(s=>s.code===parsed.code);
    const badge = !parsed
      ? '<span class="pill neutral">檔名不符命名規則</span>'
      : known ? `<span class="pill good">${parsed.code} ${known.name}</span>` : `<span class="pill neutral">${parsed.code}（新股票）</span>`;
    return `
      <div style="border-top:0.5px solid var(--border);padding:8px 0">
        <div class="row" style="justify-content:space-between">
          <div>${file.name} ${badge}</div>
          <button class="small primary" id="drv-use-${file.id}" ${parsed ? '' : 'disabled'}>一鍵匯入</button>
        </div>
        <div id="drv-text-${file.id}"></div>
      </div>`;
  }).join('');

  driveFiles.forEach(file=>{
    const useBtn = document.getElementById('drv-use-'+file.id);
    if (!useBtn) return;
    useBtn.onclick = async ()=>{
      const parsed = parseDriveFilename(file.name);
      if (!parsed) return;
      const statusBox = document.getElementById('drv-text-'+file.id);
      useBtn.disabled = true; useBtn.textContent = '匯入中...';
      statusBox.innerHTML = '<div class="hint">下載並解析PDF中...</div>';
      try{
        let text = driveFileTextCache[file.id];
        if (!text) text = await downloadAndExtractText(file.id);
        const parsedData = autoParseReportText(text);
        await ensureStockRegistered(parsed.code, parsed.name);
        prefillSnap = {
          guessedDate: parsed.guessedDate,
          broker: parsed.broker,
          price: parsedData.price,
          target: parsedData.target,
          quarters: parsedData.quarters,
          annualEPS: parsedData.annualEPS
        };
        await selectStock(parsed.code);
        document.getElementById('addSnapForm').style.display = 'block';
        renderAddSnapForm();
        document.getElementById('addSnapForm').scrollIntoView({behavior:'smooth'});
        statusBox.innerHTML = `<div class="hint">已自動解析出 ${parsedData.quarters.length} 筆季度EPS、${parsedData.annualEPS.length} 筆年度EPS，請到下方表單確認後儲存。</div>`;
      }catch(e){
        statusBox.innerHTML = `<div class="hint" style="color:var(--warn)">匯入失敗：${e.message}</div>`;
      }
      useBtn.disabled = false; useBtn.textContent = '一鍵匯入';
    };
  });
}
...
document.getElementById('btnDriveConnect').onclick = async ()=>{
  ...
  try{
    await connectDrive();
    statusEl.textContent = '已連接 Google Drive。';
    document.getElementById('btnDriveScan').style.display = 'inline-block';
  }catch(e){
    statusEl.textContent = '連接失敗：' + e.message;
  }
};
document.getElementById('btnDriveScan').onclick = async ()=>{
  const statusEl = document.getElementById('driveStatus');
  statusEl.textContent = '掃描中...';
  try{
    const files = await scanDriveFolder();
    statusEl.textContent = `掃描完成，找到 ${files.length} 個PDF檔案。`;
    renderDriveFileList();
  }catch(e){
