...
// ---------------------------------------------------------------
// Daily price history hook — NOT wired to any live data source yet.
// kHist(code) is already reserved as the storage key for this.
// Expected shape saved under kHist(code): JSON array of
//   { date: 'YYYY-MM-DD', close: number }
// sorted ascending by date.
//
// To integrate your own stock price API:
//   1. Implement fetchPriceHistoryFromAPI() below to call your API
//      and return an array in the shape above.
//   2. Call savePriceHistory(code, data) with the result.
//   3. loadPriceHistory(code) / getPriceHistory(code) will then be
//      picked up automatically by the "各報告發布當下 Forward P/E"
//      chart, which overlays a continuous price line when history
//      data is present for the selected stock.
// ---------------------------------------------------------------
...
// TODO: hook up your own stock price API here.
async function fetchPriceHistoryFromAPI(code, fromDate, toDate){
  throw new Error('fetchPriceHistoryFromAPI() 尚未串接任何API — 請在 app.js 裡實作這個函式');
}
...
function renderMain(){
  ...
  area.innerHTML = `
    ...
    <div class="card">
      <h2>每日股價匯入 (選填，供API串接使用)</h2>
      <div class="hint">貼上「日期,收盤價」，一行一筆，例如：2026-01-20,275.0。儲存後，上方圖表會改用連續每日股價線取代原本只有報告當天的點狀連線。目前 ${priceHistory.length} 筆資料。</div>
      <textarea id="histCsvInput" placeholder="2026-01-20,275.0&#10;2026-01-21,278.5&#10;..."></textarea>
      <div class="row" style="margin-top:8px">
        <button class="small primary" id="btnSaveHist">儲存每日股價</button>
        <button class="small" id="btnClearHist">清除已匯入資料</button>
      </div>
    </div>
  ...
  document.getElementById('btnSaveHist').onclick = async ()=>{
    const raw = document.getElementById('histCsvInput').value.trim();
    ...
    if(!rows.length){ alert('沒有解析到有效的「日期,收盤價」資料列'); return; }
    priceHistory = rows;
    await savePriceHistory(selected, rows);
    renderMain();
  };
  document.getElementById('btnClearHist').onclick = async ()=>{
    if(!confirm('清除這檔股票已匯入的每日股價資料?')) return;
    priceHistory = [];
    await savePriceHistory(selected, []);
    renderMain();
  };
...
function renderAddSnapForm(){
  ...
  f.innerHTML = `
    <div class="row">
      <div><span class="label">報告日期</span><input type="date" id="snapDate" value="${new Date().toISOString().slice(0,10)}"></div>
      <div><span class="label">券商</span><input type="text" id="snapBroker" placeholder="凱基證券" style="width:120px"></div>
