...
// ---------------------------------------------------------------
let allRows = [];
let viewMode = 'detail'; // 'detail' | 'ranking'
...
function buildTable(rows){
  ...
  const trs = sorted.map(r=>{
    ...
    // 資料防呆：用「股價÷EPS」重新換算一次，跟CSV裡存的Forward P/E比對，
    // 對不起來很可能是整理資料時算錯（例如誤用目標價、抓錯EPS年度）。
...
// ---------------------------------------------------------------
// 排行頁：比較「最新報告給的P/E」vs「用今日股價換算的目前Forward P/E」
// 差 = 報告給予之P/E − 目前Forward P/E，由大到小排序。
// 只納入「最新一份報告在9個月內」的股票。
// ---------------------------------------------------------------
...
async function buildRankingData(statusEl){
  ...
  for (const s of codes){
    ...
    if (!latest) continue;
    if (new Date(latest.reportDate) < nineMonthsAgo) continue; // 最新報告超過9個月，排除
    if (latest.reportPE == null) continue; // 沒有報告給予之P/E，無法比較
    ...
    if (currentPrice == null) currentPrice = latest.price; // 都抓不到就退回最新報告股價
    ...
    results.push({
      ...
      reportPE: latest.reportPE, currentPE, currentPrice, diff
    });
  }
...
function buildRankingTable(results){
  ...
  return `<table><thead><tr><th>股票</th><th>最新報告日期</th><th>券商</th><th>報告給予之P/E</th><th>目前Forward P/E</th><th>差（報告P/E − 目前P/E）</th></tr></thead><tbody>${trs}</tbody></table>`;
}

function renderMain(){
  if (viewMode === 'ranking'){ renderRankingView(); return; }
...
async function renderRankingView(){
  const area = document.getElementById('mainArea');
  if (!allRows.length){
    area.innerHTML = `<div class="card"><div class="empty">尚無資料，請按上方「同步資料」從Google Drive讀取</div></div>`;
    return;
  }
  area.innerHTML = `<div class="card"><h2>全部股票排行</h2><div id="rankingStatus" class="hint">計算中...</div><div id="rankingTableArea"></div></div>`;
  ...
  document.getElementById('rankingTableArea').innerHTML = buildRankingTable(results) +
    `<div class="hint" style="margin-top:8px">差 = 最新報告給予之P/E − 用今日股價換算的目前Forward P/E。差越大，代表股價相對報告當時假設的估值越便宜（正值可能是機會，負值可能是變貴）。此排行不含刪除鍵的個股明細表，僅供篩選參考。</div>`;
}

document.getElementById('tabDetail').onclick = ()=>{
  viewMode = 'detail';
  document.getElementById('tabDetail').classList.add('primary');
  document.getElementById('tabRanking').classList.remove('primary');
  document.getElementById('stockPickerRow').style.display = 'flex';
  renderMain();
};
document.getElementById('tabRanking').onclick = ()=>{
  viewMode = 'ranking';
  document.getElementById('tabRanking').classList.add('primary');
  document.getElementById('tabDetail').classList.remove('primary');
  document.getElementById('stockPickerRow').style.display = 'none';
  renderMain();
};
