...
function buildChartSVG(rows, dailyHistory){
  ...
  const W = 640, H = 220, padL = 50, padR = 50, padT = 16, padB = 30;
  ...
  const priceVals = points.filter(p=>p.price!=null).map(p=>p.price).concat(dailyHistory.map(h=>h.close));
  ...
  points.forEach(p=>{
    ...
    if (x - lastLabelX >= minLabelGap){
      svg += `<text x="${x}" y="${H-6}" text-anchor="middle" font-size="8" fill="var(--text3)" transform="rotate(-40 ${x} ${H-6})">${p.reportDate.slice(2)}</text>`;
      lastLabelX = x;
    }
  });
  svg += `</svg>`;
...
function renderMain(){
  ...
  area.innerHTML = `
    <div class="card">
      ...
      <div class="hint">${priceHistory.length > 1 ? '灰線(右側股價軸)為FinMind抓取的每日實際股價走勢' : '灰線(右側股價軸)為每份報告當時股價（尚無每日股價資料）'}；藍點(左側P/E軸)是該份報告發布當下算出的Forward P/E，點與點之間不連線。紅點代表當下EPS估值為負或為零，本益比無意義。</div>
    </div>
    <div class="card">
