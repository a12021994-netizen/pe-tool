...
function drawDateAxis(points, xOf, H, showLabels){
  ...
  points.forEach(p=>{
    ...
    if (x - lastLabelX >= minLabelGap){
      // 用垂直文字（旋轉90度）取代斜體，避免斜體文字在邊緣被裁切掉開頭字元
      svg += `<text x="${x}" y="${H-6}" text-anchor="end" font-size="7" fill="var(--text3)" transform="rotate(-90 ${x} ${H-6})">${p.reportDate.slice(5)}</text>`;
      lastLabelX = x;
    }
  });
  return svg;
...
function buildPEChartSVG(rows, dailyHistory){
  ...
  const W = 640, H = 200, padL = 44, padR = 16, padT = 14, padB = 42;
...
function buildPriceChartSVG(rows, dailyHistory){
  ...
  const W = 640, H = 150, padL = 44, padR = 16, padT = 14, padB = 40;
  const plotW = W - padL - padR, plotH = H - padT - padB;
  const allDates = points.map(p=>p.date.getTime()).concat(dailyHistory.map(h=>new Date(h.date).getTime()));
  const minDate = Math.min(...allDates);
  const maxDate = Math.max(...allDates);
